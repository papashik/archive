...
- Функция _filter__()_ принимает в качестве аргументов функцию и последовательность, которую необходимо отфильтровать(функция, передаваемая в _filter()_ должна возвращать значение True / False) и возвращает \<filterobject\>, который можно затем переконвертировать в список – с помощью функции _list()_.
...
**Функции для работы с регулярными выражениями** живут в модуле **re**.
Основные функции:
